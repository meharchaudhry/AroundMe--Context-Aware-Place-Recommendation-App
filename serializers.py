...
class PlaceVisitedSerializer(serializers.ModelSerializer):
    class Meta:
        model = Place
        fields = ['id', 'name', 'neighborhood', 'latitude', 'longitude', 'categories', 'rating']

class UserVisitedPlacesSerializer(serializers.ModelSerializer):
    place = PlaceVisitedSerializer(read_only=True)

    class Meta:
        model = Interaction
        fields = ['place', 'type', 'dwell_time_sec', 'timestamp']